...
function addon:CreateSettingsPanel()
    ...
    local currentProfileName = UltraCursorFXDB.profiles[addon.currentZoneProfile] and
        UltraCursorFXDB.profiles[addon.currentZoneProfile].name or "World"
    local currentProfileColor = UltraCursorFXDB.profiles[addon.currentZoneProfile] and
        UltraCursorFXDB.profiles[addon.currentZoneProfile].color or {0.0, 1.0, 1.0}
    ...
    for _, profileInfo in ipairs(profileButtons) do
        ...
        local profileColor = UltraCursorFXDB.profiles[profileInfo.key] and
            UltraCursorFXDB.profiles[profileInfo.key].color or {0.0, 1.0, 1.0}
    ...
    enableCB:SetScript("OnClick", function(self)
        ...
        if UltraCursorFXDB.enabled then
            addon.frame:SetScript("OnUpdate", function(_, elapsed) addon:OnUpdate(elapsed) end)
        else
    ...
    local pointsSlider = CreateSlider("Points", "Trail Length (Points)", yPos, 10, 100, 1,
    ...
    local sizeSlider = CreateSlider("Size", "Particle Size", yPos, 10, 100, 1,
        "Bigger = more visible | Smaller = subtle")
    ...
    local glowSlider = CreateSlider("GlowSize", "Glow Intensity", yPos, 10, 150, 1,
        "Larger = brighter glow around particles")
    ...
    local smoothSlider = CreateSlider("Smooth", "Smoothness", yPos, 0.05, 0.50, 0.01,
    ...
    local pulseSlider = CreateSlider("Pulse", "Pulse Speed", yPos, 0.5, 5.0, 0.1,
        "How fast the trail pulses/breathes")
    ...
    colorBtn:SetScript("OnClick", function(self)
        ShowColorPicker(function(r, g, b) self.texture:SetColorTexture(r, g, b) end)
    end)
    yPos = yPos - 40
    ...
    local clickParticlesSlider = CreateSlider("ClickParticles", "Click Particles", yPos, 4, 24, 1,
    ...
    local clickSizeSlider = CreateSlider("ClickSize", "Click Particle Size", yPos, 20, 100, 1,
        "Size of click particles")
    ...
    local clickDurationSlider = CreateSlider("ClickDuration", "Click Effect Duration", yPos, 0.2, 2.0, 0.1,
        "How long click effects last")
    ...
    local cometSlider = CreateSlider("CometLength", "Comet Tail Length", yPos, 1.0, 5.0, 0.1,
        "Length multiplier for comet tail")
    ...
    exportEditBox:SetScript("OnEnterPressed", function(self) self:ClearFocus() end)
    exportEditBox:SetScript("OnEditFocusGained", function(self) self:HighlightText() end)
    exportEditBox:SetScript("OnEditFocusLost", function(self) self:HighlightText(0, 0) end)
